	...
	if (manualLearnset?.levelup) {
		...
		for (const learnableMove of manualLearnset.levelup) {
			if (learnableMove.level <= level) {
				// FIX: Ensure the move name from the data file is converted to a proper ID.
...
export const commands: ChatCommands = {
	rpg: {
		...
		learneggmove(target, room, user) {
			...
			const speciesId = toID(pokemon.species);
			// The rawMoveId comes from the button and might have hyphens.
			...
			// FIX: Sanitize the move ID before adding it to the Pokémon's moveset.
